...
function addHotelSection(Hotel, display_case){
    ...
    if(Hotel.alwaysTop != 'true'){
        ...
        labelHotel.innerHTML = Hotel.roomAvailable+' Room Available';
    }
    else{
        labelHotel.setAttribute('class', 'aa-tag sold-out');
        labelHotel.innerHTML = 'Only '+Hotel.roomAvailable+' Rooms Available';
    }
    ...
    let div_bedding = document.createElement('div');
    ...
    bookButton.addEventListener('click', function(e){
        ...
        let nameHotel = decodeHtml(this.parentNode.previousSibling.childNodes[0].innerHTML);
        document.getElementById('title-hotel-Bookingmodal').innerHTML = nameHotel;
        ...
        let AccomodationTOTAL = '<br><br>Accomodation total: '+nbDays+" x "+priceNight+" = £"+(nbDays*priceNight);

        //We say we cannot apply the discount
        let NoDiscount = (discount(nameHotel) == 0)? '': '<br>Your IP Adress indicates that your area is not eligible for the discount.';
        let PlatformFee = '<br><br>Hotel booking platform fees (10%) = 0.1 x '+(nbDays*priceNight)+" = £"+  (0.1*nbDays*priceNight).toFixed(2);
        let Breakfast = '<br><br>Breakfast total = '+ nbDays + " * 30 = £"+(30*nbDays);
        let BigTotalWithoutBreakfast = '<br>Total VAT included = £'+(1.1*nbDays*priceNight).toFixed(2);
        let BigTotalWithBreakfast = '<br>Total VAT included = £'+(1.1*nbDays*priceNight+ nbDays*30).toFixed(2);
        pricing.innerHTML = AccomodationTOTAL+NoDiscount+PlatformFee+'<hr>'+BigTotalWithoutBreakfast;

        checkboxBreakfast.addEventListener('change', function(e){
            e.preventDefault();

            if(this.checked){
                pricing.innerHTML= AccomodationTOTAL+NoDiscount+PlatformFee+Breakfast+'<hr>'+BigTotalWithBreakfast;
            }
            else
                pricing.innerHTML= AccomodationTOTAL+NoDiscount+PlatformFee+'<hr>'+BigTotalWithoutBreakfast;
        });
