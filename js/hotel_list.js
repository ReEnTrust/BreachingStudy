...
const priceBreakfast = 30;
const dayDuration = 1000*3600*24;
const duration = 1000 * 60 * 5;
const btnsSlider = document.getElementsByClassName("aa-top-slider-btn");
const display_case = document.getElementById('div-display-hotel');
const sortingDiv = document.getElementById('div-sorting-option');
...
searchButton.addEventListener('click', function(e){
    ...
    let formSort = document.createElement("select");
    let optionHighestPrice = document.createElement("option");
    let optionHighestScore = document.createElement("option");
    let optionLowestScore = document.createElement("option");
    let optionLowestPrice = document.createElement("option");
    optionHighestPrice.value = "HP";
    optionLowestPrice.value = "LP";
    optionHighestScore.value = "HR";
    optionLowestScore.value = "LR";

    optionHighestPrice.innerHTML = "Highest Price First";
    optionLowestPrice.innerHTML = "Lowest Price First";
    optionHighestScore.innerHTML = "Highest Reviews First";
    optionLowestScore.innerHTML = "Lowest Reviews First";

    formSort.appendChild(optionHighestPrice);
    formSort.appendChild(optionLowestPrice);
    formSort.appendChild(optionHighestScore);
    formSort.appendChild(optionLowestScore);
    sortingDiv.appendChild(formSort);

    //By default it is on Highest price first
    sortByPrice(hotelToDisplay, false);
    changeSomeResults(hotelToDisplay);
    Display(hotelToDisplay);

    formSort.addEventListener('change', function(e){
        switch (formSort.value) {
            case "HP":
                sortByPrice(hotelToDisplay, false);
                changeSomeResults(hotelToDisplay);
                Display(hotelToDisplay);
            ...
            case "LP":
                sortByPrice(hotelToDisplay, true);
                changeSomeResults(hotelToDisplay);
                Display(hotelToDisplay);
                break;

            case "HR":
                sortByReview(hotelToDisplay, false);
                changeSomeResults(hotelToDisplay);
                Display(hotelToDisplay);
                break;

            default:
                sortByReview(hotelToDisplay, true);
                changeSomeResults(hotelToDisplay);
                Display(hotelToDisplay);
        }
...
function changeSomeResults(TD){
    //We change the second one after the featured with the last one
    if(TD.length >= 8){
        let temp = TD[7];
        TD[7] = TD[TD.length-1];
        TD[TD.length-1] = temp;
...
function sortByReview(TD, ascending){
    for(let i= TD.length -1; i>= 1; i--){
        for(let j= 0; j<i ; j++){
            if((TD[j].alwaysTop != 'true') && (TD[j+1].alwaysTop != 'true')){

                if(ascending == true){

                    if((TD[j].averageReview == 'No reviews') || (parseFloat(TD[j+1].averageReview) < parseFloat(TD[j].averageReview)))
                    {
                        let temp = TD[j+1];
                        TD[j+1]= TD[j];
                        TD[j] = temp;
...
function sortByPrice(TD, ascending){
    for(let i= TD.length -1; i>= 1; i--){
        for(let j= 0; j<i ; j++){
            if((TD[j].alwaysTop != 'true') && (TD[j+1].alwaysTop != 'true')){

                if(ascending == true){
                    if(parseInt(TD[j+1].price) < parseInt(TD[j].price))
                    {
                        let temp = TD[j+1];
                        TD[j+1]= TD[j];
                        TD[j] = temp;
