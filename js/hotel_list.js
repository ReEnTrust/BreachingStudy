...
function addHotelSection(Hotel, display_case){
    ...
    bookButton.addEventListener('click', function(e){
        ...
        let AccomodationTOTAL = '<br><br>Accomodation total: '+nbDays+" x "+nbRooms+" x "+priceNight+" = £"+(nbDays*priceNight*nbRooms);

        //We say we cannot apply the discount
        let priceBreakfast = 110;
        let NoDiscount = (discount(nameHotel) == 0)? '': '<br><i>The discount is not valid for the chosen dates.</i>';
        let PlatformFee = '<br><br>Hotel booking platform fees (10%) = £'+  (0.1*nbDays*priceNight*nbRooms).toFixed(2);
        let Breakfast = '<br><br>Breakfast total = £'+(priceBreakfast*nbDays*nbRooms);
        let BigTotalWithoutBreakfast = '<br>Total VAT included = <h3>£'+(1.1*nbDays*priceNight*nbRooms).toFixed(2) +'</h3>';
        let BigTotalWithBreakfast = '<br>Total VAT included = <h3>£'+(1.1*nbDays*priceNight*nbRooms+ nbDays*priceBreakfast*nbRooms).toFixed(2) +'</h3>';
        ...
        NextStepButton.addEventListener('click',function(e){
            e.preventDefault();
            swal("Your location indicates that you may not be able to afford this hotel. Do you wish to continue ?", {
                buttons: {
                    ...
                    if (value === "catch") {
                        ...
                        proceedPaymentBut.addEventListener('click', function(e){
                           ...
                           if((this.parentNode.previousSibling.previousSibling.childNodes[0].childNodes[2].value == '') || (this.parentNode.previousSibling.previousSibling.childNodes[0].childNodes[6].value == '')){
                               swal("Error!", "You forgot to fill something.", "error");
                           }
                           else{
